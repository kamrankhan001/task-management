...
const { deleteRecord } = useDeleteRecord("/api/admin/groups/" + `${props.id}`);
...
const distroyRecord = async () => {
    try {
        const response = await deleteRecord();
        if (response.success) {
            emit("recordDeleted", "Group Deleted Successfully");
            closeModal();
        }
        if (response.error) {
    ...
    <div>
        <div class="text-right">
            ...
            <fwb-modal v-if="isShowModal" @close="closeModal">
                ...
                <template #body>
                    <p class="text-left">
                        Are you sure you want to delete this group permently?
                    </p>
                </template>
                <template #footer>
