...
const ViewGroup = defineAsyncComponent(() => import("./View.vue"));
const CreateGroup = defineAsyncComponent(() => import("./Create.vue"));
const EditGroup = defineAsyncComponent(() => import("./Edit.vue"));
const DeleteGroup = defineAsyncComponent(() => import("./Delete.vue"));

const { allRecord } = useAllRecord("/api/admin/groups");
        ...
        <div class="px-4 lg:px-12">
            <div class="bg-white dark:bg-gray-800 relative sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    ...
                    <CreateGroup @record-created="handleRecords" />
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" v-if="records?.data?.length > 0">
                        ...
                        <tbody>
                            <tr class="border-b dark:border-gray-700" v-for="record in records.data" :key="record.id">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white capitalize">
                                    {{ record.name }}
                                </th>
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="inline-flex rounded-md shadow-sm" role="group">
                                        <ViewGroup :id="record.id" />
                                        <EditGroup :id="record.id" @record-updated="handleRecords" />
                                        <DeleteGroup :id="record.id" @record-deleted="handleRecords" />
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation" v-if="links.length > 1">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing <span class="font-semibold text-gray-900 dark:text-white">{{ records.meta.from }}-{{ records.meta.to }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ records.meta.total }}</span>
                    </span>
                    ...
                    <ul class="inline-flex items-stretch -space-x-px">
                        ...
                        <li v-for="link in links" :key="link.label">
                            <button @click="fetchRecords(link.label, query)" class="flex items-center justify-center text-sm py-2 px-3 leading-tight border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:hover:text-white" :class="link.active == true ? 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-white' : 'text-gray-500 bg-white dark:text-gray-400 dark:hover:bg-gray-700'">
                                {{ link.label }}
                            </button>
                        </li>
                        <li>
